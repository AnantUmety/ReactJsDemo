...
const SchoolStatus = ({ resellerId = "" }) => {
  ...
  useEffect(() => {
    if (resellerList && resellerList.length > 0 && resellers.length === 0) {
      let res = [];
      resellerList.forEach((item) => {
        let data = {};
        data.label = item.resellerFirstName + " " + item.resellerLastName;
        data.value = item.resellerId;
        res.push(data);
      });
      setResellers(res);
    ...
    if (resellerId == "") {
      col = [
        ...
        },
        {
          ...
          Cell: (cellInfo) => {
            const { row } = cellInfo;
            const fullName = `${row.original.schoolAdminFirstName} ${row.original.schoolAdminLastName}`;
            return <>{fullName}</>;
          },
        },
        {
  ...
  const hitGetOrganization = (
    ...
    if (resellerId)
      dispatch(
        getOrganiztionByResellerIdAction(resellerId, searchText, limit, page)
      );
    else {
      let resellerIds = [];
      if (filterApplied)
      ...
      dispatch(getOrganizationAction(searchText, limit, page, resellerIds));
    }
  };
            ...
            <>
              ...
              {/* {schools.length > 0 ? (
                <Fragment> */}
                  ...
                  />
                  {schools.length > 0 &&<TableBoxPagination
                    page={page}
                    limit={limit}
                    data={schools}
