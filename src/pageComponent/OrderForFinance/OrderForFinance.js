...
const OrderForFinance = () => {
  ...
  const commentHandler = (financeComment) => {
   ...
   const approveOrderFunc = (obj)=>{
    setIsApproved(!isApproved)
    setIsApprovedData(obj)
   }

  const approveHandler = (row)=>{
    return(
      <div className={styles.approveMain}>
          <HiOutlineDotsVertical onClick={()=> approveOrderFunc(row)} />
      </div>
    )
  }
  ...
  return (
    <Fragment>
          ...
          <Grid container>
            <Grid item xs={12} className={styles.helloOrder}>
              ...
              <TabContainer value={isTabOpen} isOpen={isTabOpen}>
                {/* {isTabOpen == 0 ? "sjkfdhsdjkla;jhsvg" : "dfghjkliurtyfgh"} */}
                <Fragment>
                  {orderList ? (
                    <>
                      <Table
      ...
      {isApproved && (
        <ModalGlobal heading="Order approve" activeState={isApproved} onClick={setIsApproved}>
            <ApprovedBox refreshData={refreshData} setIsApproved={setIsApproved} isApprovedData={isApprovedData} />
        </ModalGlobal>
      )}
...
OrderForFinance.propTypes = {};

export default OrderForFinance;
